...
#for SUBJ in $(seq $NSUBJ); do
   ...
   #apptainer run --env MLM_LICENSE_FILE=${PORT}@${SERVER} pipeline_matlab.sif singlesubject-$SUBJ-input singlesubject-$SUBJ-output participant   
   #apptainer run pipeline.sif singlesubject-$SUBJ-input singlesubject-$SUBJ-output participant
#done

# MERGE
#apptainer run oras://ghcr.io/siesta-eu/mergesubjects.sif:${VER} singlesubject-merged-output $(for SUBJ in $(seq 1 $NSUBJ); do echo -n "singlesubject-$SUBJ-output "; done)

#for SUBJ in `seq $NSUBJ`; do
    #apptainer run oras://ghcr.io/siesta-eu/leaveoneout.sif:${VER} singlesubject-merged-output leaveoneout-$SUBJ-input $SUBJ
    #apptainer run --env MLM_LICENSE_FILE=${PORT}@${SERVER} pipeline_matlab.sif singlesubject-$SUBJ-output leaveoneout-$SUBJ-output group    
