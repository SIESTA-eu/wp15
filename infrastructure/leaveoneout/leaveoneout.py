...
def leaveoneout_(input_dir, output_dir, participant_id):
    ...
    try:
        ...
        if participant_id in participant_ids:
            ...
            for root, dirs, files in os.walk(input_dir):
                if participant_id in root.split(os.sep):
                    # skip any directories that contain the participant ID to be left out
                    continue
                ...
                for root, dirs, files in os.walk(output_base, topdown=False):
                    for dir in dirs:
                        ...
                        if str(participant_id[:4]) in dir_path and str(participant_id) in dir_path:
                            try:
            ...
            print(f"{participant_id} has been successfully excluded.")        
        else:
            print(f"Error: participant_id[{participant_id}] does not exist in {input_dir}.")  
        return True
...
def main(args=None):
    ...
    pid = leaveoneout_(input_dir, output_dir, participant_id)
